...
void quadToImuMsg(const QuadrotorSimulator::Quadrotor& quad, sensor_msgs::Imu& imu);
    ...
    // 从四旋翼获取当前状态：包括位置、速度、姿态（四旋翼的旋转矩阵R）、角速度等。

    // 从控制命令获取期望的控制力和姿态：使用force和orientation。

    // 计算姿态误差：通过比较当前姿态和期望姿态，计算出姿态误差。

    // 计算角速度误差：比较当前角速度和通过期望姿态计算的理论角速度。

    // 根据误差计算控制力矩：使用姿态误差和角速度误差，结合控制增益kR和kOm，计算出需要施加的力矩。

    // 计算每个电机的推力：根据总推力和力矩，使用四旋翼的动力学模型计算每个电机需要提供的推力。

    // 转换推力为电机转速：根据电机和螺旋桨的动力学特性，将推力转换为电机的转速。
