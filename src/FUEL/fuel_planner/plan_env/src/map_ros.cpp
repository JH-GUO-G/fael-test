...
void MapROS::cloudPoseCallback(const sensor_msgs::PointCloud2ConstPtr& msg,
  ...
  // camera_q_ = Eigen::Quaterniond(-pose->pose.orientation.x, pose->pose.orientation.w,
  ...
  for (const auto& point : cloud.points) {
    Eigen::Vector3d point_in_lidar(point.x, point.y, point.z);  // 点在雷达坐标系中的位置
    Eigen::Vector3d point_in_world = camera_q_ * point_in_lidar + camera_pos_;  // 转换到世界坐标系
    ...
    cloud_in_world.points.push_back(point_world);
  }

  map_->inputPointCloud(cloud_in_world, num, camera_pos_);
...
void MapROS::publishMapLocal() {//在local_bound_min_当前帧点云范围内且z高度box_max_内的占据地图,且z在可视化范围,-2-10
  pcl::PointXYZ pt;
  pcl::PointCloud<pcl::PointXYZ> cloud;
