...
// void SDFMap::initMap(ros::NodeHandle& nh,Eigen::Vector3d& pos) {
...
void SDFMap::resetBuffertwo(const Eigen::Vector3d& pos) {
  std::cout << "map_min_boundary_: (" << mp_->map_min_boundary_.x() << ", "
          << mp_->map_min_boundary_.y() << ", "
          << mp_->map_min_boundary_.z() << ")" << std::endl;

  std::cout << "map_max_boundary_: (" << mp_->map_max_boundary_.x() << ", "
          << mp_->map_max_boundary_.y() << ", "
          << mp_->map_max_boundary_.z() << ")" << std::endl;
  resetBuffer(mp_->map_min_boundary_, mp_->map_max_boundary_);

  // mp_->map_min_boundary_ = mp_->map_origin_+ pos;
  // mp_->map_max_boundary_ = mp_->map_min_boundary_ + mp_->map_size_;
  md_->local_bound_min_ = Eigen::Vector3i::Zero();
  md_->local_bound_max_ = mp_->map_voxel_num_ - Eigen::Vector3i::Ones();
  cout<<"-------------------------------clear____________________"<<endl;
  std::cout << "map_min_boundary_: (" << mp_->map_min_boundary_.x() << ", "
          << mp_->map_min_boundary_.y() << ", "
          << mp_->map_min_boundary_.z() << ")" << std::endl;
...
void SDFMap::inputPointCloud(
  ...
  // ROS_WARN_STREAM(" point_num "<<point_num);
  ...
  for (int i = 0; i < point_num; ++i) {
    ...
    double distance_to_camera = sqrt(pow(pt_w(0) - camera_pos(0), 2) +
                                     pow(pt_w(1) - camera_pos(1), 2) +
                                     pow(pt_w(2) - camera_pos(2), 2));

    // 如果距离小于0.25米，跳过该点云
    if (distance_to_camera < 0.25 ) {
    // if (distance_to_camera < 0.25 ) {
        continue;
    }
    ...
    if (!isInMap(pt_w)) {
      ...
      if (length > mp_->max_ray_length_) {
        pt_w = (pt_w - camera_pos) / length * mp_->max_ray_length_ + camera_pos;
        // if (pt_w[2] < 0.2 || length < 0.5) continue;
        if (pt_w[2] < 0.2 ) continue;
        tmp_flag = 0;
      } else
        tmp_flag = 1;//1表示射线范围内
    ...
    for (int k = 0; k < 3; ++k) {
      ...
      update_max[k] = max(update_max[k], pt_w[k]);//点云的边界
    }
    ...
    caster_->nextId(idx);
    while (caster_->nextId(idx))//沿线的设为free
      setCacheOccupancy(toAddress(idx), 0);
  }
  // ROS_WARN_STREAM(" point_num "<<numon);
  ...
  posToIndex(update_min - bound_inf, md_->local_bound_min_);//帧点云的边界加上膨胀
  ...
  for (int k = 0; k < 3; ++k) {
    ...
    md_->update_max_[k] = max(update_max[k], md_->update_max_[k]);//所有点云的边界
  }

  while (!md_->cache_voxel_.empty()) {
    int adr = md_->cache_voxel_.front();
    md_->cache_voxel_.pop();
    if (md_->occupancy_buffer_[adr] > mp_->min_occupancy_log_ - 1e-3){
    continue;
    }
