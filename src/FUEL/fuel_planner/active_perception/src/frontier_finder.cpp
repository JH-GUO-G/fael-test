...
//根据点云帧的大小结合地图大小得到更新区域，后方不变，前方更新
  ...
  for (int k = 0; k < 3; ++k) {
    search_min[k] = max(search_min[k], box_min[k]);//box即是探索大小
  ...
  while (!cell_queue.empty()) {
    ...
    for (auto nbr : nbrs) {
      ...
      if (frontier_flag_[adr] == 1 || !edt_env_->sdf_map_->isInBox(nbr) ||
          !(knownfree(nbr) && isNeighborUnknown(nbr)))//周围新发现的forn
        continue;
...
bool FrontierFinder::isFrontierCovered() {
  ...
  auto checkChanges = [&](const list<Frontier>& frontiers) {
    for (auto ftr : frontiers) {
      ...
      const int change_thresh = min_view_finish_fraction_ * ftr.cells_.size();//frotier是否被探索
      int change_num = 0;
      for (auto cell : ftr.cells_) {
        Eigen::Vector3i idx;
        edt_env_->sdf_map_->posToIndex(cell, idx);
