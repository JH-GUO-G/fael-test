...
int FastExplorationManager::planExploreMotiontwo(
    const Vector3d& pos, const Vector3d& vel, const Vector3d& acc, const Vector3d& yaw,bool& expl,geometry_msgs::PoseStamped& way_pose_)
...
//   const Vector3d& pos, const Vector3d& vel, const Vector3d& acc, const Vector3d& yaw)
  ...
  std::cout << "start pos2: " << pos.transpose() << ", vel: " << vel.transpose()
  ...
  // Search frontiers and group them into clusters
    ...
    if (ed_->points_.size() > 1) {
      ...
      if (ep_->refine_local_) {
        ...
        int knum = min(int(indices.size()), ep_->refined_num_);//indices全局视点
        ...
        ed_->next_pos = ed_->points_[indices[0]];
        ed_->next_yaw = ed_->yaws_[indices[0]];
      }
      ...
      if (ep_->refine_local_) {
        ...
        ed_->next_pos = ed_->n_points_[0][min_cost_id];
        ed_->next_yaw = n_yaws[0][min_cost_id];
        ed_->refined_points_ = { ed_->next_pos };
        ed_->refined_views_ = { ed_->next_pos + 2.0 * Vector3d(cos(ed_->next_yaw), sin(ed_->next_yaw), 0) };
      } else {
        ed_->next_pos = ed_->points_[0];
        ed_->next_yaw = ed_->yaws_[0];
      }
    } else
      ROS_ERROR("Empty destination.");

    std::cout << "Next view: " << ed_->next_pos.transpose() << ", " << ed_->next_yaw << std::endl;
    ROS_WARN_STREAM("Next view  "<< ed_->next_pos.transpose());

  }
  else{
  Eigen::Vector3d point(way_pose_.pose.position.x,
                        way_pose_.pose.position.y,
                        way_pose_.pose.position.z);
...
//   const Vector3d& pos, const Vector3d& vel, const Vector3d& acc, const Vector3d& yaw)
  ...
  if(expl== true){//-----------------------------------
  // Search frontiers and group them into clusters
    frontier_finder_->searchFrontiers();//寻找更新重复的前沿，并分割大块， 更新区域，找种子，扩散切割，作tmp

    double frontier_time = (ros::Time::now() - t1).toSec();
    t1 = ros::Time::now();

    // Find viewpoints (x,y,z,yaw) for all frontier clusters and get visible ones' info
    frontier_finder_->computeFrontiersToVisit();//对tmp前沿生成视点,有则排序视点作为前沿
    frontier_finder_->getFrontiers(ed_->frontiers_);
    frontier_finder_->getFrontierBoxes(ed_->frontier_boxes_);
    frontier_finder_->getDormantFrontiers(ed_->dead_frontiers_);
...
//---------------------------------------------------------------------------------------------
  ...
  double diff = fabs(ed_->next_yaw - yaw[0]);
  double time_lb = min(diff, 2 * M_PI - diff) / ViewNode::yd_;
  
  
  std::cout << "way pos: " << ed_->next_pos.transpose() << std::endl; 
  
  // // Generate trajectory of x,y,z
  // //导航的搜索
  // planner_manager_->kino_path_finder_->reset();
  // // int FastExplorationManager::planExploreMotion(
  // if (planner_manager_->kino_path_finder_->search(pos,vel,acc, ed_->next_pos,end_v,true) == KinodynamicAstar::NO_PATH) {//////////形参修改
  //   ROS_ERROR("[kino replan]: kinodynamic search fail!");
  //   // return FAIL;
  //   planner_manager_->kino_path_finder_->reset();
  //   if (planner_manager_->kino_path_finder_->search(pos,vel,acc, ed_->next_pos,end_v,false) == KinodynamicAstar::NO_PATH) {
  ...
  // }
  // }
  // ed_->path_next_goal_ = planner_manager_->kino_path_finder_->getKinoTraj(0.01);//-----------------------------------------函数名
  
  
  // 探索的搜索
  planner_manager_->path_finder_->reset();
  if (planner_manager_->path_finder_->search(pos, ed_->next_pos) != Astar::REACH_END) {
  ...
  planner_manager_->planYawExplore(yaw, ed_->next_yaw, true, ep_->relax_time_);
